...
const Contact = () => {
    ...
    const onSubmit = async (event) => {
      ...
      formData.append("access_key", "f73d8f9b-b7da-49e3-b13b-004154bda42f");
        ...
        <form onSubmit={onSubmit} className='max-w-2xl mx-auto text-grey-600 pt-8'>
            <div className='flex flex-wrap'>
                <div className='w-full md:w-1/2 text-left'>
                    Your Name
                <input className='w-full border border-grey-300 rounded py-3 px-4 mt-2' 
                type="text"
                 name='Name'
            ...
            <div className='my-6 text-left'>
                Message
                <textarea className='w-full border-grey-300 rounded py-3 px-4 mt-2 h-48 resize-none' 
                name="Message" 
                placeholder='Message' required>
                </textarea>
            </div>
            <button className='bg-blue-600 py-2 px-12 mb-10 rounded-full'>{result ? result : "Send Message"}</button>
        </form>
